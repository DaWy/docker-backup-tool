...
En el config.py hi podem configurar el path de destí per defecte i algunes coses mes!
...
* `./docker.backup.py -c [LLISTA_CONTENIDORS]`
...
Netejara tots els backups anteriors als dies indicats en l'arxiu. Conservará nomes 1 backup de cada contenidor per nom.
...
L'script consultará els backups dins del **backup_path** (indicat al config.py o el indicat amb el parámetre -d) i ens demanará quin dels backups disponibles volem restaurar.

Indiquem el backup amb el número y la imatge quedara restaurada en el daemon local de docker. 

La imatge tindra el nom de <NOM_CONTENIDOR:DATA_BACKUP> per a que la poguem localitzar. Despres només faltaria indicar en el contenidor que ens interessi que estiri aquesta imatge. 
